 ...
 */
export const CEIL = (amount, stepBelow, stepAbove) => {
    if (stepBelow === amount) {
        return stepBelow;
    }
    else {
 ...
 */
export const FLOOR = (amount, stepBelow, stepAbove) => {
    if (stepAbove === amount) {
        return stepAbove;
    }
    else {
 ...
 * plus infinity.
 */
export const HALF_UP = (val, stepBelow, stepAbove) => {
 ...
 */
export const BANKERS_ROUNDING = (amount, stepBelow, stepAbove, stepIndex) => {
...
export default class Rounder {
     ...
     */
    constructor(decade, base = 10, roundingStrategy = BANKERS_ROUNDING) {
        if (decade.length === 0)
            throw new Error('Decade must contain at least one step');
        ...
        if (decade[decade.length - 1] >= decade[0] * base) {
            throw new Error('The last step of the decade must be smaller than the first step multiplied by the base');
        }
        this.decade = decade;
        this.base = base;
    ...
    search(amount, roundingStrategy = this.roundingStrategy) {
        if (amount === 0)
            return 0;
        ...
        return roundingStrategy(amount, currentDecade[i - 1], currentDecade[i], i + segmentIndex * this.decade.length);
    }
    findMultiplier(amount) {
        let multiplier = 1;
        let segmentIndex = 0;
